...
int main()
{	
	lint n=0, u=0, v=0, maxDist=0, maxIndex=0;
	scanint(n);
	vector< vector<lint> > tree(n);
	vector<lint> dfsDist(n, -1);
	queue<lint> dfs;
	...
	while(!dfs.empty())
	{
		...
		FOR(i, 0, s(tree[p]))
		{
            if(dfsDist[tree[p][i]] == -1)
            {
                dfsDist[tree[p][i]] = dfsDist[p] + 1;
                if(dfsDist[p]+1 > maxDist)
                    maxIndex = tree[p][i];
                dfs.push(tree[p][i]);
            }
		}
    ...
    fill(dfsDist.begin(), dfsDist.end(), -1);
    dfsDist[maxIndex] = 0;
    dfs.push(maxIndex);
    while(!dfs.empty())
    {
