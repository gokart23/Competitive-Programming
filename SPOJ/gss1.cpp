...
ulint n=0, m=0;
ulint segTree[200000], values[50000];

void buildTree(ulint node, ulint start, ulint end)
{
	if(start == end)
	...
	segTree[node] = max(segTree[2*node+1]+segTree[2*node+2], max(segTree[2*node+1], segTree[2*node+2]));
}
