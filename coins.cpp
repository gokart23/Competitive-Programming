...
#define FOR(i,j,n) for(ulint i=j;i<n;i++)
#define pb push_back
#define s(v) v.size()
...
ulint recCompute(ulint a)
{
    if(a <= 1e6)
        return dp[a];
    else
        return max(a, recCompute(a/2) + recCompute(a/3) + recCompute(a/4));
}
...
{	_
	...
	FOR(i, 4, 1e6+2)
	{
        ulint t = dp[i/2] + dp[i/4] + dp[i/3];
		dp.pb(i>t?i:t);
		//cout<<i<<": "<<dp[floor(i/2)]<<"+"<<dp[floor(i/3)]<<"+"<<dp[floor(i/4)]<<" -- "<<dp[i]<<endl;
	}
